...
import { UpdateCompanyInputType, ReturnType } from "./types";
...
export const handler = async (
  inputData: UpdateCompanyInputType
): Promise<ReturnType> => {
  const httpResult = await httpPut<UpdateCompanyInputType, string>(
    `/api/v1/companies/${inputData.id}`,
    inputData
  );
  const fieldErrors: FieldErrors<UpdateCompanyInputType> = {
    id: [],
    name: [],
    isActive: [],
    legalName: [],
...
export const updateCompany = createSafeAction(UpdateCompanySchema, handler);
